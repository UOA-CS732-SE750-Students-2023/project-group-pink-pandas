...
import { patient, user, team, task } from '../data/dummy-data';
...
async function clearDatabase() {
    ...
    console.log(
        `Cleared database (removed ${usersDelete.deletedCount} users).`
    );
    const teamsDelete = await Team.deleteMany({});
    console.log(
        `Cleared database (removed ${teamsDelete.deletedCount} teams).`
    );
...
async function addTask() {
    for (const data of task) {
        const dbMon = new Task(data);

        await dbMon.save();
        console.log(`Task saved! _id = ${dbMon._id}`);
    }
}
